(define-library (srfi 69)
  ...
  (define (%string-hash s ch-conv bound)
    (let ((strlen (string-length s)))
      (letrec ((rec
                (lambda (hash index)
                  (if (>= index strlen)
                      (floor-remainder hash bound)
                      (rec (floor-remainder (+ (* 37 hash)
                                      (char->integer
  ...
  (define (hash obj . maybe-bound)
    (let ((bound (if (null? maybe-bound) *default-bound* (car maybe-bound))))
      (cond ((integer? obj) (floor-remainder obj bound))
            ((string? obj) (string-hash obj bound))
            ((symbol? obj) (symbol-hash obj bound))
            ((real? obj) (floor-remainder (+ (numerator obj) (denominator obj)) bound))
            ((number? obj)
             (floor-remainder (+ (hash (real-part obj)) (* 3 (hash (imag-part obj))))
                     bound))
            ((char? obj) (floor-remainder (char->integer obj) bound))
            ((vector? obj) (vector-hash obj bound))
            ((pair? obj) (floor-remainder (+ (hash (car obj)) (* 3 (hash (cdr obj))))
                                 bound))
            ((null? obj) 0)
            ((not obj) 0)
  ...
  (define (vector-hash v bound)
    (let ((len (vector-length v)))
      (letrec ((rec
                (lambda (hashv index)
                  (if (>= index len)
                      (floor-remainder hashv bound)
                      (rec (floor-remainder (+ (* 257 hashv) (hash (vector-ref v index)))
                                   *default-bound*)
                           (+ index 1))))))
        (rec 571 0))))
  ...
  (define (make-hash-table . args)
    (let* ((comparison (if (null? args) equal? (car args)))
           ...
           (size (if (or (null? args) (null? (cdr args)) (null? (cddr args)))
                     *default-table-size*
                     (caddr args)))
           (association (or (and (eq? comparison eq?) assq)
  ...
  (define make-integer-hash-table
    (make-hash-table-maker = floor-remainder))
  ...
  (define (%hash-table-walk proc entries)
    (let loop ((index (- (vector-length entries) 1)))
      (if (< index 0)
          (values) ;; unspecified
  ...
  (define (hash-table-ref/default hash-table key value)
